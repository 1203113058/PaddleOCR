...
> 以下是在本机 macOS 上快速安装并运行 PaddleOCR 的步骤。

## 环境要求

- Conda (miniforge3)
- Python 3.10（已验证环境：`paddleocr310`）
...
> 如果还没有创建环境，先执行：
> ```bash
> conda create -n paddleocr310 python=3.10 -y
> conda activate paddleocr310
> pip install paddlepaddle==3.2.0 -i https://www.paddlepaddle.org.cn/packages/stable/cpu/
> pip install paddleocr pypdfium2 openpyxl pandas gradio
> ```
...
首次运行自动从 ModelScope 下载，缓存在本机（所有 conda 环境共享）：

```
~/.paddlex/official_models/
├── PP-OCRv5_server_det/    ( 84 MB)  文字检测
├── PP-OCRv5_server_rec/    ( 81 MB)  文字识别
├── PP-DocLayout_plus-L/    (124 MB)  文档版面分析
├── PP-DocBlockLayout/      (124 MB)  文档块检测
├── PP-FormulaNet_plus-L/   (701 MB)  公式识别
├── PP-Chart2Table/         (1.3 GB)  图表转表格
└── ...（共 20 个模型，合计 3.7 GB）
```

> 删除某个子目录后，下次运行会自动重新下载该模型。

---

## 三、PDF 表格识别 → 三文件输出（推荐）

脚本路径：`/Users/project/QMS/PaddleOCR/QMS/pdf2excel.py`

工作流程：PDF → 图片渲染 → PP-OCRv5 识别 → 行列聚类 → 三文件输出

### 基础配置（首次运行前可选）

```bash
cd /Users/project/QMS/PaddleOCR/QMS
python -u config_ui.py        # 打开 Gradio 配置页面 http://localhost:7860
                              # 可设置默认输出目录、置信度阈值、行分组容忍度
                              # 保存后写入 config.toml，下次运行自动读取
```

配置文件路径：`/Users/project/QMS/PaddleOCR/QMS/config.toml`

```toml
[output]
output_dir = "output"       # 默认输出目录（绝对/相对路径均可）

[ocr]
threshold = 0.991           # 低置信度阈值（低于此值标红）
row_tol = 15                # 行分组 Y 差值容忍度（px）
```
...
- **选择文件…** — 通过文件对话框选择一个或多个 PDF（支持批量）
- **选择目录…** — 通过目录对话框选择输出位置
- 调整置信度阈值和行分组容忍度后点击 **▶ 开始识别**
- 运行日志实时输出到窗口底部；完成后点击 **打开目录** 查看结果
...
# 使用 config.toml 默认参数运行
python -u pdf2excel.py -i "/Users/project/QMS/PaddleOCR/文件/机械性能复检报告.pdf"

# 指定输出目录（覆盖 config.toml 默认值）
python -u pdf2excel.py -i "/Users/project/QMS/PaddleOCR/文件/机械性能复检报告.pdf" -o output

# 自定义置信度阈值和行分组容忍度
python -u pdf2excel.py -i "/Users/project/QMS/PaddleOCR/文件/机械性能复检报告.pdf" \
  --threshold 0.95 --row-tol 20
```

> **`-u` 参数**：强制不缓冲输出，进度实时显示，推荐始终加上。

### 输出文件

生成目录：`/Users/project/QMS/PaddleOCR/QMS/output/`（或 config.toml 中配置的路径）

```
output/
├── 机械性能复检报告_table.xlsx
│     还原格式可编辑表格，每页一个 Sheet（第1页/第2页…）
│     行列与原 PDF 对齐，跨行跨列单元格自动合并（如 B1:D2）
│
├── 机械性能复检报告_data.xlsx
│     数据存储表格，每行对应一条 OCR 识别结果
│     列：页码 | 序号 | 识别文本 | 置信度 | 左上X/Y | 右下X/Y | 行号 | 列号 | 列号止 | 行号止
│     低置信度行标红背景
│
├── 机械性能复检报告_page_1.jpg          PDF 渲染原图
├── 机械性能复检报告_page_1_ocr_res_img.jpg   OCR 全量可视化（绿框/红框）
├── 机械性能复检报告_page_1_low_conf.jpg      低置信度线框标注（红色边框）
├── 机械性能复检报告_page_1_conf_heat.jpg     置信度热力图（差距越大红色越深）
└── 机械性能复检报告_page_1_res.json
      增强 JSON，在原有字段基础上追加 row_nums / col_nums
```
...
|------|------|-----------|
| `-i` / `--input` | PDF 文件路径 | 必填 |
| `-o` / `--output` | 输出目录 | `config.toml → output.output_dir` |
| `--threshold` | 低置信度阈值（0~1） | `config.toml → ocr.threshold` |
| `--row-tol` | 行分组 Y 差值容忍度（px） | `config.toml → ocr.row_tol` |
...
```bash
# PP-OCRv5 识别本地图片
paddleocr ocr -i /path/to/image.png \
...
for res in ocr.predict(input="/path/to/image.png"):
    res.print()
